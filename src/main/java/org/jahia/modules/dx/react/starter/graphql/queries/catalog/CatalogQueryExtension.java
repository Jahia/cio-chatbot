...
import org.jahia.modules.dx.react.starter.graphql.catalog.InputConfig;
import org.jahia.modules.dx.react.starter.graphql.catalog.InputConfigBuilder;
import org.jahia.modules.dx.react.starter.graphql.catalog.inputVariant;
...
public class CatalogQueryExtension {
    ...
    public static List<CioProduct> getProducts(@GraphQLNonNull @GraphQLName("conversationId") String conversationId,
        ...
        List<inputVariant> selectedVariants = ((InputConfig) inputConfigBuilder.getInputVariables().get("config")).getFacets().getShopByVariants().getSelectedVariants();
        if (!CollectionUtils.isEmpty(selectedVariants)) {
            String color = null;
            for (inputVariant selectedVariant : selectedVariants) {
                if (selectedVariant.getName().equals("style")) {
                    color = selectedVariant.getValue().toLowerCase();
                }
            }
            if (color != null) {
                logger.info("Calling CDP with favorite color '{}' profile {} conversation {}", color, profileId, conversationId);
                saveFavoriteColor(color, profileId, om);
            }
        }
    ...
    @GraphQLField
    public static String getFavoriteColor(@GraphQLName("profileId") String profileId) throws Exception {
